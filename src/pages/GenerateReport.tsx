...
const GenerateReport = () => {
  ...
  const generatePDF = () => {
    ...
    const reportTypeText = reportConfig.reportType === "billing" ? "Billing" : "Utilization";
    doc.text(`${reportConfig.frequency.charAt(0).toUpperCase() + reportConfig.frequency.slice(1)} ${reportTypeText} Report - ${reportConfig.provider.toUpperCase()}`, pageWidth / 2, yPos, { align: "center" });
    yPos += 20;
    ...
    // Add instances section if it's a utilization report
    if (reportConfig.reportType === "utilization" && reportConfig.instances.length > 0) {
      doc.setFontSize(14);
    ...
    // Add RDS instances if any for utilization reports
    if (reportConfig.reportType === "utilization" && reportConfig.rdsInstances.length > 0) {
      yPos += 10;
